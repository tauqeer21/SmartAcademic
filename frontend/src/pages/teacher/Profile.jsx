...
export default function TeacherProfile() {
  ...
  const handleChangePw = async e => {
    ...
    if (newPw===oldPw)  { toast.error('Must be different from current'); return; }
    setSaving(true);
    try {
  ...
  const INFO = [
    ...
    { label:'Teacher ID', val:user?.roll_no||'—',                                 icon:'🪪', color:'#2F80ED', bg:'#EBF4FF' },
    { label:'Role',       val:'Teacher',                                           icon:'🎓', color:'#27AE60', bg:'#E8FBF0' },
    { label:'Joined',     val:profile?.created_at?new Date(profile.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}):'—', icon:'📅', color:'#F2994A', bg:'#FFF3E8' },
    { label:'Classes',    val:profile?.classes?.join(', ')||currentClass?.name||'—', icon:'🏫', color:'#6347D1', bg:'#EEF0FF' },
    { label:'Subjects',   val:profile?.subjects?.join(', ')||currentClass?.subjects||'—', icon:'📚', color:'#27AE60', bg:'#E8FBF0' },
  ];
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:860 }}>
      ...
      <div className="card fade-up" style={{ padding:0, overflow:'hidden' }}>
        <div style={{ height:90, background:'linear-gradient(135deg,#7B61FF,#5A3EC8)', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ position:'absolute', bottom:-30, left:80, width:120, height:120, borderRadius:'50%', background:'rgba(255,255,255,0.05)' }}/>
        </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:20, padding:'16px 0 24px' }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:8, marginTop:6, flexWrap:'wrap' }}>
                <span style={{ fontSize:11, fontWeight:700, padding:'3px 12px', borderRadius:'50px', background:'#EEF0FF', color:'#6347D1' }}>🎓 Teacher</span>
                <span style={{ fontSize:11, fontWeight:700, padding:'3px 12px', borderRadius:'50px', background:'#E8FBF0', color:'#27AE60' }}>ID: {user?.roll_no}</span>
                {currentClass && <span style={{ fontSize:11, fontWeight:700, padding:'3px 12px', borderRadius:'50px', background:'#EBF4FF', color:'#2F80ED' }}>🏫 {currentClass.name}</span>}
              </div>
            </div>
          </div>
            ...
            {INFO.map(i=>(
              <div key={i.label} style={{ padding:'14px 16px', background:i.bg, borderRadius:16, display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div style={{ minWidth:0 }}>
                  ...
                  <div style={{ fontSize:12, fontWeight:700, color:i.color, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={i.val}>{i.val}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card-body" style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', background:'var(--surface2)', borderRadius:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div>
                <div style={{ fontSize:13, fontWeight:700, color:'var(--text1)' }}>Theme</div>
                <div style={{ fontSize:11, color:'var(--text3)' }}>Currently: {theme==='dark'?'Dark':'Light'} Mode</div>
              </div>
            </div>
      ...
      <div className="card fade-up-d2">
        <div className="card-header"><div><div className="card-title">🔐 Change Password</div><div className="card-sub">Keep your account secure</div></div></div>
          ...
          <form onSubmit={handleChangePw}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14, marginBottom:16 }}>
              ...
              <div className="field">
                ...
                <div style={{ position:'relative' }}>
                  <input className="inp" type={showNew?'text':'password'} placeholder="Min 6 characters" value={newPw} onChange={e=>setNewPw(e.target.value)} style={{ paddingRight:40 }}/>
              ...
              <div className="field">
                ...
                <input className="inp" type="password" placeholder="Repeat new password" value={confPw} onChange={e=>setConfPw(e.target.value)} style={{ borderColor:confPw&&confPw!==newPw?'var(--red)':'' }}/>
